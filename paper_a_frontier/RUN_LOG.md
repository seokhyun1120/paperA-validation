...
코드: `frontier.py` (원본 커밋 b811573 → placeholder 2개만 교체; 통계 로직 무수정)
...
- 검증 후 print 제거, N_SIM=10,000 원복, 그림은 커밋본(N_SIM=10k)으로 복원 —
  frontier.py는 커밋 9d35e1b와 동일(git diff 없음).
...
N_SIM=100,000, n=360, δ=0. e-process 로직은 frontier.py 커밋 9d35e1b와 동일
...
## §8.1 적응탐색 시뮬레이터 (2026-07-06, `sim/`, 엔진 커밋 a8a4129)
...
- 참고: `sim_results/*.parquet`와 `run_simE*.log`는 repo .gitignore 정책
  ...
  SEED 체계 + 엔진 커밋(a8a4129) 고정이므로 `python3 -m sim.run_E{1..5}`로
  bit-동일 재생성 가능.

## §8.1 잔여 작업 2차 (2026-07-06, 엔진 커밋 a6efa22 기준 실행)
...
## 심사 대응 실험 R1·R2 (2026-07-06, 엔진 커밋 f059eb9 기준 실행)
...
## 심사 대응 3차 — 6건 (2026-07-07, `review/rev3_*` + `sim/run_R4·R5`, 실행 기준 commit d809d82)
...
## 심사 대응 4차 — 3건 (2026-07-07, `review/rev4_*`, 실행 기준 commit d9ac163)
...
## 심사 대응 5차 — 게이트·재료 5건 (2026-07-07, 실행 기준 commit ad0ba0d)
...
출처: gaussian = 커밋 9d35e1b 커널 SEED=0 (600-grid 재현 `run_rev3_fig1.log`),
t5 = 커밋 d9ac163 실행 `rev4_m4_t5_long.csv` SEED=0.
...
## 심사 대응 5차 추가 — 마이크로 2건 (2026-07-07, 실행 기준 commit 528b29d)
...
## 공개 repo 이력 필터링 공지 (2026-07-07)

동반 공개 repo(paperA-validation)의 git 이력을 `git filter-repo`로 1회 재작성 —
사유: 워크스페이스 repo에 함께 있던 **무관 미발표 프로젝트들의 비의도 노출 제거**
(Paper A와 무관, 본 로그의 어떤 결과에도 관여하지 않음). 유지 경로:
`paper_a_frontier/`, README.md, manifest.json, AGENTS.md, .gitignore.

- **파일 내용(blob)·데이터 sha256은 전부 불변** — 커밋 ID만 교체됨 (커밋 ID는
  repo 전체 트리의 해시이므로). 커밋 메시지·저자·날짜·순서 보존.
- 본 로그와 `appendix_M8_audit.tex`의 커밋 해시는 **필터 후 해시로 일괄 갱신**됨.
  2026-07-07 이전 원고/회신문 사본의 구 해시는 repo 루트 `HASH_MAP.md`의
  구→신 매핑표로 대조 가능.
- 필터 전 원본 repo는 비공개로 전량 보존 (bundle 아카이브 포함) — 에디터 요청 시
  제공 가능.
