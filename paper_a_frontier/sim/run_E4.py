"""E4 — Envelope stress (Corollary 2 실측).
...
측정: 실현 SupFDR vs 이론 상계 alpha*xi. 상계 내면 PASS, 초과 시 STOP.
...
def main():
    ...
    for m_env, xi in CELLS:
        ...
        rows.append({"m_env_reg": m_env, "xi": xi, "n_run": N_RUN,
                     "supfdr": supfdr, "mc_se": se, "bound_alpha_xi": bound,
                     "pass": supfdr <= bound, "disc_total": int(n_discs.sum()),
                     "any_disc_rate": float((n_discs > 0).mean())})
        print(f"  m_env={m_env} xi={xi}: SupFDR={supfdr:.5f} (SE {se:.5f}) "
              f"vs α·ξ={bound:.3f} -> {'PASS' if supfdr <= bound else 'FAIL'} "
              f"({time.time()-t0:.0f}s)", flush=True)
    summ = pd.DataFrame(rows)
    path = common.write_parquet(summ, "E4.parquet")
    print("\n" + summ.to_string(index=False))
    all_pass = bool(summ["pass"].all())
    print(f"\nE4 {'PASS — 전 셀 α·ξ 상계 내' if all_pass else '*** FAIL: STOP·보고 ***'}"
    ...
    ax.plot(xs, ALPHA * xs, "--", color="#D55E00", lw=2, label="theory bound α·ξ")
    ...
    ax.set_xlabel("true variance inflation ξ (Var = ξ·m_env²)")
    ax.set_ylabel("empirical SupFDR")
    ax.set_title(f"E4 — envelope stress, all-null, J={J_REG}, D={D} "
                 f"(N_RUN={N_RUN}/cell)")
